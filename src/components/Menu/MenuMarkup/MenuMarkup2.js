...
class MenuMarkup extends Component{
  render(){
    const userInfo = cmConfig.getUserInfoByToken(this.props.token);
    const { mobileVersion, activeLinkKey, onLinkClick, className } = this.props;
    return(
    <Menu
      theme={'light'}
    ...
    >
      ...
      <Menu.Item key={`/${_.toLower(userInfo.dept)}/upload`}>
            <Link onClick={onLinkClick} to={`/${_.toLower(userInfo.dept)}/upload`}>Upload</Link>
          </Menu.Item>
...
export default connect(mapStateToProps)(MenuMarkup);
